...
LIB_DIRECTORY = "src/multilspy"
IGNORED_DIRECTORIES = []
"""existing files in ignored directories will be restored via 'git checkout' on pull/push, but new files will be added"""
FULLY_IGNORED_VERSIONED_DIRECTORIES = []
"""fully in ignored versioned directories will be restored to original state after push/pull via git checkout"""
FULLY_IGNORED_UNVERSIONED_DIRECTORIES = []
"""fully in ignored unversioned directories will be backed up and restored to original state after push/pull"""
IGNORED_FILE_GLOB_PATTERNS = []
"""files matching ignored glob patterns will be restored via 'git checkout' on pull/push"""
LIB_NAME = "multilspy"
...
class OtherRepo:
    ...
    def __init__(self, name, branch, path_to_lib):
        self.pathToLibInThisRepo = os.path.abspath(path_to_lib)
    ...
    @classmethod
    def prepare_restoration_of_ignored_files(cls, base_dir: str):
        """
        ...
        if os.path.exists(cls.SYNC_BACKUP_DIR):
            shutil.rmtree(cls.SYNC_BACKUP_DIR)
        os.mkdir(cls.SYNC_BACKUP_DIR)

        # backup ignored, unversioned directories
        for d in FULLY_IGNORED_UNVERSIONED_DIRECTORIES:
            if os.path.exists(d):
                shutil.copytree(d, os.path.join(cls.SYNC_BACKUP_DIR, d))

        os.chdir(cwd)

    @classmethod
    def restore_ignored_files(cls, base_dir: str):
        """
        :param base_dir: the directory containing the lib directory, to which ignored paths are relative
        """
        cwd = os.getcwd()
        ...
        for d in FULLY_IGNORED_VERSIONED_DIRECTORIES + FULLY_IGNORED_UNVERSIONED_DIRECTORIES:
            if os.path.exists(d):
                print("Removing overwritten content: %s" % d)
                shutil.rmtree(d)

        # restore directories and files that can be restored via git
        for d in IGNORED_DIRECTORIES + FULLY_IGNORED_VERSIONED_DIRECTORIES:
            restoration_cmd = "git checkout %s" % d
            print("Restoring: %s" % restoration_cmd)
            os.system(restoration_cmd)
        for pattern in IGNORED_FILE_GLOB_PATTERNS:
        ...
        for d in FULLY_IGNORED_UNVERSIONED_DIRECTORIES:
            if os.path.exists(os.path.join(cls.SYNC_BACKUP_DIR, d)):
                print("Restoring from backup: %s" % d)
                shutil.copytree(os.path.join(cls.SYNC_BACKUP_DIR, d), d)
